...
export default function SRecog_SVGDetector(props: { type: string; }) {
  ...
  fn_recognizorInfo.curr_speaker = redux_recog.currentSpeaker;  // Remarks: React cannot update current speaker, if we only pass the state into the function
  // Prev Page
  ...
  if (props.type === "Recognizer") {
    ...
    if (redux_recog.recognizerStatus === "started" || redux_recog.recognizerStatus === "resumed")
      ...
      return (
        <button onClick={() => {
          SRecogStarter(dispatch); dispatch(setRecognizer());
        }}>
          <div>
            <svg xmlns="http://www.w3.org/2000/svg" className="recog_icon_inactive" viewBox="0 0 16 16">
              <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16" />
